...
public class SandpilePanel extends JPanel implements ActionListener, Serializable, Runnable {
	...
	private boolean repaintEdges = true;
	...
	public void paintComponent(Graphics g) {
		...
		if (labels) {
			...
			for (int i = 0; i < vertexData.size(); i++) {
				...
				g.drawString(String.valueOf(sand), scaleCoordinate(v[0] - VERT_RADIUS / 2), scaleCoordinate(v[1] + VERT_RADIUS / 2));
			}
		}
		if (selectedVertex >= 0) {
	...
	public void paintVertices(Graphics g) {
		for (int i = 0; i < vertexData.size(); i++) {
			...
			if (useBufferedImages) {
				//int r = VERT_RADIUS;
				g.drawImage(vertexImages[colorNum], scaleCoordinate(v[0] - VERT_RADIUS), scaleCoordinate(v[1] - VERT_RADIUS), new Color(0, 0, 0, 0), null);
			} else {
